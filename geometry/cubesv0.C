...
#if defined(__CINT__) && !defined(__MAKECINT__)
{
   ...
   gSystem->CompileMacro("cubesv0.C");
   protov2();
}
#else
...
void cubesv0(){
	...
	Float_t phi=0.0;
    Float_t rc=0.0;
    Float_t x_rc[1024]={0.0};
    Float_t y_rc[1024]={0.0};
	Int_t i, bin;
	Float_t LL = 10.0;//mm
  ...
  Int_t bin_num[257]={0};
   ...
   // std::stringstream sector1_name;
    ...
    TFile *file = new TFile ("cubes_geo_hires.root","RECREATE");
  ...
  for(Int_t k=0;k<n_strips;k++){
     ...
     //32*32 squares, 4 cornners for each
